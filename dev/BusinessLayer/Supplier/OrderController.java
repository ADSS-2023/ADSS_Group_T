...
public class OrderController {
    ...
    public void loadOrders() throws Exception {
        List<OrderDTO> ordersList = orderDalController.findAll("supplier_order", OrderDTO.class);
        ...
        for (OrderDTO orderDTO : ordersList) {
            ...
            if (orderDTO.getIsOrderSupplied().equals("true")) // if the order already supplied
                orders.add(order);
    ...
    private List<OrderProduct> loadOrderProducts(int orderId) throws SQLException {
    ...
    public void executeTodayOrders() throws SQLException {
        ...
        for (OrderBusiness order : ordersForToday) {
            ...
            OrderDTO newOrderDTO = new OrderDTO(oldOrderDTO.getOrderNum(), oldOrderDTO.getSupplierNum(),
                    oldOrderDTO.getContactName(), oldOrderDTO.getContactNumber(),
                    oldOrderDTO.getOrderDate(), oldOrderDTO.getSupplierAddress(),
                    oldOrderDTO.getDestinationAddress(), "true", oldOrderDTO.getDaysToDeliver(), oldOrderDTO.getConstantDay());
            ...
            for (OrderProduct product : order.getProducts()) {
                if (product.getProductName().equals(item.getProductName()) &&
                    ...
                    if (quantityToChange > 0) { // if there is a need to add - if an existing supplier has enough to add
                        ...
                        // if the current product has the  amount to be reduced
    ...
    public void updateRegularItem(OrderProduct product, String productName, String manufacturer, int supplierNum) throws Exception {
        ...
        product.orderProductDTO=newOrderProductDTO;
    }
